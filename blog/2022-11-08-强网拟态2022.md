...
[附件仓库](https://github.com/Nova-Noir/NovaNo1r-pwn-challenges/强网拟态2022/)
...
- 首先造一个 `double free`，之后，在这个 `double free` 堆块上修改 `fd` 指针到其它大小的堆块上再次形成 UAF，并修改这个堆块的大小。在修改大小的同时，可以把该函数的 `fd` 指针再改成该位置形成新的 `double free`。
...
- 在我们释放这个堆块后它被塞入 `unsortedbin` 中，由于 `double free`，我们可以再添加一个新的堆块，修改 `7f0` 这个堆块的 `fd` 为 `_IO_2_1_stdout_` 的指针，通过打 IO 泄露 `libc`。
