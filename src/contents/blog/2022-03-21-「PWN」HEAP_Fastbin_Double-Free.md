...
整体原理比较简单，在[ctf-wiki](https://ctf-wiki.org/pwn/linux/user-mode/heap/ptmalloc2/fastbin-attack/#fastbin-double-free)上可以看到。主要就是因为fastbin在检查时只检查链表头部且释放时不清除`prev_in_use`
...
为了方便，我们关闭ASLR并使用`glibc-2.23`作为动态解释器
...
类似于[fasbin_dup.c](#fastbin_dup.c)，它申请了3个`chunk`，这里我们不再赘述。直接跳到34行——也就是Double Free完成之后

重新申请一个d，它拿去了`a`所表示的chunk，此时`a`所表示的chunk是我们可控的fastbin
...
值得注意的是，由于`size`要相同，我们第0个chunk的大小应该比其他chunk大`0x10`
...
`libc_base`出来了，可是如何调用`system`还是困难的。因为保护是`FULL RELRO`，所以我们没办法改写`GOT表`，即使能改，因为我们没有办法传参，也没办法过去（除非我们找到one-gadget然后再想办法在栈上写）
