...
const fs = require('fs');
import { LoadContext, Plugin, PluginOptions, PluginContentLoadedActions } from '@docusaurus/types'
...
export default async function AISummary(context: LoadContext, options: {}) {
    // ...
    return {
      name: 'ai-summary',
      async loadContent() {
          ...
          for (const file of files) {
            ...
            if (summaryExists) {
              ...
              if (summaryStats.mtime >= blogStats.mtime) {
                ...
                summaries[blogRoute][fileName] = fs.readFileSync(summaryFilePath, 'utf-8');
                continue;
              }
            }
      ...
      async contentLoaded({content, actions}: {content: any; actions: PluginContentLoadedActions}) {
